...
from utils import   load_text, load_paths, load_quads, myConvert,cal_metrics, CosineEmbeddingLoss, load_history_paths, reshape_ranking_data
...
parser.add_argument('--learning_rate', type=float, default=1e-5,
                    help='learning rate')
parser.add_argument('--model', type=str, default='model/ALBERT',
                    help='sentence transformer model name on Hugging Face website (huggingface.co)')
parser.add_argument('--tokenizer', type=str, default='model/ALBERT',
...
if args.model_load_file is None:
    model_load_file=os.path.join(f"save/{args.dataset}",f"entity_prediction_{args.mode}/best_val.pth")
else:
    model_load_file=args.model_load_file

if args.model_save_dir is None:
    model_save_dir=os.path.join(f"save/{args.dataset}", f"entity_prediction_{args.mode}/")
else:
...
model = TKG_Model(tokenizer_name=args.tokenizer,model_name=args.model,num_rels=data.num_rels*2,device=device)
        ...
        for step, batch in enumerate(train_data_loader):
            ...
            with autocast():
                for s1, s2, tgt in zip(sentence1, sentence2, targets):
                    ...
                    #     sim[i][:pnums[i]//2] *= model.weight[torch.tensor(rels[i])]
                    #     sim[i][pnums[i]//2:pnums[i]] *= model.weight[torch.tensor(rels[i])]
                    # sim = torch.stack(
